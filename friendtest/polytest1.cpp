...
void test_size(polynomial& test);
// Postcondition: The user has been prompted for the size value. The
// size of the polynomial is then changed to match that size.
...
void print_menu()
{
    ...
    cout << "R - use the reserve function" << endl;
    cout << "+ - view A + B" << endl;
    cout << "- - view A - B" << endl;
    cout << "  -   -   -   -   -   -   -   -   -   -   -   -" << endl;
...
void view_poly(const polynomial& test)
{
    for(int i = test.degree(); i > 0; i--)
    {
        cout << test.coefficient(i) << "X^" << i << " + ";
    }
    cout << test.coefficient(0);
    cout << "\n";
}
...
void test_size(polynomial& test)
{
    ...
    cout << "Enter new size for polynomial: ";
    cin >> size;

    test.reserve(size);

    cout << "New size of polynomial = " << size;
}
